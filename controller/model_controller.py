    ...
    try:
        ...
        if not model_metadata['feature_names']:
            ...
            if n_features > 0:
                ...
                return jsonify({'error': 'No se pudo extraer información de features del modelo. Proporciona feature_names en metadata.'}), 400
        ...
        if not is_pipeline and not scaler_temp_path:
            # Si no es Pipeline y no hay scaler, error
            logger.warning("Modelo no es Pipeline y no se proporcionó scaler")
            # Permitimos igual, asumiendo que el usuario sabe lo que hace
...
@model_bp.route('/admin/model/<model_id>', methods=['DELETE'])
@jwt_required()
def delete_model(model_id):
    ...
    try:
        ...
        return jsonify({'message': f'Modelo {model_id} eliminado'}), 204
    except Exception as e:
